...
function removeToCart(event){
  ...
  /*decrease by one the order's  number in the cart*/
  /*var numberOfOrder=localStorage.getItem('orderInCart');
...
window.onload=function(){
  ...
  setInterval(function(){
    ...
    var x=localStorage.getItem("firstPic");
    var y=localStorage.getItem("firstName");
    var z=localStorage.getItem("firstPrice");
    var dsg=localStorage.getItem("designation");
    var sz=localStorage.getItem("sizeValue");
    var itemAdded=document.createElement("div");
    var cartBox=document.querySelector(".cart-box");
    itemAdded.classList.add("cart-item");
    var content="<img src="+x+" alt='Désolé, l\'image ne charge pas'><div class='cart-details'><span class='name'>"+y+"</span></br><span class='price'>"+z+"</span></br>Taille:<span class='size'>"+sz+"</span></br>Qté:<span class='quantity'>"+dsg+"</span><a href='Product.html'><input class='edit-item' type='button' Value='Modifier' onclick='editItem(event)' required></a><input class='delete-item' type='button' Value='Supprimer' onclick='removeToCart(event)' required></div>";
    itemAdded.innerHTML=content;
    cartBox.appendChild(itemAdded);
    document.querySelector(".cart-item img").src=x
    /*allow to save the information in the box to be display
     when the page will load another time*/
    var perm=localStorage.getItem("theCart");
    if(perm){
      var all=perm+cartBox.innerHTML;
    }else{
      var all=cartBox.innerHTML;
    }
    localStorage.setItem("theCart",all);
    ...
    var d=Date.now();//the time passed since 1970 January first , in milliseconds
    console.log(d);
    ...
    if(localStorage.getItem("state")=="true"){
      ...
      if(checkName==checkName2 && checkSize==changeSize2){
        ...
        //request to put here, decrease the stock
        var all=cartBox.innerHTML;
        localStorage.setItem("theCart",all);
        perm=localStorage.getItem("theCart");
        cartBox.innerHTML=perm;
...
function readyToPay(){
  ...
  for(s;s<namesReady.length;s++){
    ...
    /*help us to store the infos relative to our shopping in a div
    element which will be displayed in the Purchase page*/
